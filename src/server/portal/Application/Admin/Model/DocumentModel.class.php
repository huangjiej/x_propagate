...
class DocumentModel extends Model{
    ...
    public function remove(){
        ...
        $base_list = $this->where($map)->field('id,model_id,media_id')->select();
        //删除扩展模型数据
        $base_ids = array_column($base_list,'id');
        //孤儿数据
        $orphan   = get_stemma( $base_ids,$this, 'id,model_id,media_id');
        ...
        foreach ($all_list as $key=>$value){
            if(!empty($value['media_id'])){
                ...
                //删除单条微信素材
                $api->setData(['media_id'=>$value['media_id'],'path'=>'wechat/article/delete'])->send('wechat/async/run');
            }
